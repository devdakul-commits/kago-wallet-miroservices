import { ChatMessage } from '../../models/chatModels.js';
import { query } from '../../../shared/db/index.js';

export class VendorChatRepository {
  async getMessagesByUser(firebaseUid: string) {
    const result = await query(
      `SELECT id, firebase_uid, message, is_from_user, created_at FROM support_chat WHERE firebase_uid = $1 ORDER BY created_at ASC`,
      [firebaseUid]
    );
