...
export class InventoryController {
  ...
  listItems = (req: Request, res: Response) => {
    ...
    if (!vendorUid) return sendError(res, 400, 'Vendor UID required');
  ...
  reserveItem = (req: Request, res: Response) => {
    ...
    if (!reservation) return sendError(res, 400, 'Unable to reserve item');
  ...
  getReservationStatus = (req: Request, res: Response) => {
    const orderId = String(req.params.order_id ?? '');
    if (!orderId) return sendError(res, 400, 'Order ID required');
