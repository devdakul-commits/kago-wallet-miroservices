...
export class LaundryController {
    service;
    constructor(service = new LaundryService()) {
        this.service = service;
    }
    listClothes = async (_req, res) => {
        try {
    ...
    createOrder = async (req, res) => {
        ...
        if (payload.items.length === 0)
            return sendError(res, 400, 'No items in order');
        try {
    ...
    getOrderDetails = async (req, res) => {
        ...
        if (!firebaseUid || !orderId)
            return sendError(res, 400, 'firebase_uid and order_id required');
        try {
    ...
    cancelOrder = async (req, res) => {
        ...
        try {
            ...
            if (result === null)
                return sendError(res, 404, 'Order not found');
            if (result === -1)
                return sendError(res, 400, 'Cannot cancel order with current status');
